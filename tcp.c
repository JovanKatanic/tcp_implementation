...
int valid_numbers_check(struct connection *conn, struct tcphdr *tcp_header, uint32_t segment_len, int tun_fd)
{
    ...
    {
        if (!is_between(recieved.nxt - 1, seq, recieved.nxt + sent.wnd - 1) && !is_between(recieved.nxt - 1, seq + segment_len - 1, seq_win))
        {
...
void establish_connection(struct tcphdr *tcp_header, struct connection_manager *manager, int tun_fd, struct quad *quad, hashtable_kv_t key)
{
...
void handle_connection(struct tcphdr *tcp_header, struct connection_manager *manager, int tun_fd,
                       hashtable_entry_t *connection_state, uint32_t segment_len, unsigned char *data)
{
    ...
    case FinWait1:
        printf("FinWait1\n");
        if ((tcp_header->th_flags & (TH_ACK | TH_FIN)) == 0)
        { // todo test if i can use tcp_header->th_flags == TH_FLAG instead of this
...
void *packet_loop(void *arg)
{
    ...
    while (1)
    {
        ...
        if (ip_header->protocol == IPPROTO_TCP)
        {
            ...
            {
                establish_connection(tcp_header, manager, args->tun_fd, quad, key);
            }
            else
            {
                handle_connection(tcp_header, manager, args->tun_fd, connection_state, segment_len, data);
            }
        }
    }
}
